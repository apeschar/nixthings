...
in {
  ...
  in
    lib.mkMerge [
      (lib.mkIf (cfg.certName != null) {
        security.acme.certs.${cfg.certName}.postRun = lib.mkAfter ''
          ${pkgs.systemd}/bin/systemctl start --no-block postgresql-certs.service
        '';
      })
      {
        systemd.services = {
          postgresql-certs = ifCert {
            wants = ["acme-${cfg.certName}.service"];
            after = ["acme-${cfg.certName}.service"];
            serviceConfig = {
              Type = "oneshot";
              ExecStart = copyCerts;
            };
